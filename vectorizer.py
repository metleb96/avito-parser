...
class AvitoVectorizer:
    ...
    def process_data(self) -> Dict[str, any]:
        ...
        try:
            ...
            logger.info(
                f"Найдено {
                    len(data_to_vectorize)} записей для векторизации.")

            if not data_to_vectorize:
                logger.warning("Нет данных для обработки.")
                return result

            # Генерация эмбеддингов (batch_size можно уменьшить для экономии
            # памяти)
            logger.info("Генерация эмбеддингов (это может занять время)...")
            embeddings = self.model.encode(
                data_to_vectorize, batch_size=32, show_progress_bar=True)
            ...
            logger.info(
                f"Векторизация завершена успешно! Обработано {
                    len(structured_output)} элементов.")

        except FileNotFoundError:
            logger.error(
                f"Файл {
                    self.input_file} не найден. Сначала запустите processor.py.")
        except Exception as e:
